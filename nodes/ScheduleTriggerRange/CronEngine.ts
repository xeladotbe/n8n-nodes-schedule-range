 ...
 *
 * Scope, deliberately: numeric fields only (no month/weekday names), no
 * `L`/`W`/`#` special characters. That covers every expression this node
 * itself generates and the vast majority of hand-written cron strings.
 */
...
export function parseCronExpression(expression: string): ParsedCronFields {
	...
	return {
		...
		daysOfWeek: parseField(dayOfWeek, 0, 6, true),
