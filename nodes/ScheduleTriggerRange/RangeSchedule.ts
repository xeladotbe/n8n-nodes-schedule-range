...
export function ruleToCronExpression(rule: IntervalRule): string {
	...
	switch (rule.field) {
		...
		case 'weeks': {
			...
			// Deliberately fires every matching weekday every week - plain cron
			// has no field for "every Nth week". `weeksInterval` is applied
			// separately, at tick time, via isInTriggerWeek().
 ...
 */
export function parseRuleRange(rule: Pick<IntervalRule, 'range'>): {
	start: Date | null;
	end: Date | null;
} {
	return {
		start: rule.range?.startDate ? new Date(rule.range.startDate) : null,
		end: rule.range?.endDate ? new Date(rule.range.endDate) : null,
	};
}
...
/**
 * True if `date` falls in a week that should fire for a weekly rule with the
 * given `weeksInterval` (e.g. 2 = every other week). Weeks are counted in
 * fixed 7-day blocks from the Unix epoch (1970-01-01 UTC) - a deterministic,
 * stateless anchor chosen so the pattern never shifts depending on when the
 * workflow happens to be activated. This is unlike n8n's native Schedule
 * Trigger, which tracks the ISO week number modulo 52 and can skip or repeat
 * a week across a year boundary.
 */
export function isInTriggerWeek(date: Date, weeksInterval: number): boolean {
	if (weeksInterval <= 1) return true;
	const MS_PER_WEEK = 7 * 24 * 60 * 60 * 1000;
	const weekIndex = Math.floor(date.getTime() / MS_PER_WEEK);
	return weekIndex % weeksInterval === 0;
}

/** Throws a plain Error if a rule's own start/end range is inverted (start >= end). */
export function assertValidRange(rule: Pick<IntervalRule, 'range'>): void {
